...
public class Tela {
    ...
    private void criarCompetidores() {
        ...
        competidores.add(new Corredor("Lucas", 2));
        competidores.add(new Corredor("Gabriel", 7));
        competidores.add(new Corredor("Henrique", 4));
        competidores.add(new Corredor("Maria", 6));
        competidores.add(new Corredor("Joao", 2));
    ...
    private void iniciarCorrida() {
        ...
        if (distanciaDaCorrida > 0) {
            ...
            Thread atualizarThread = new Thread(
                new Runnable() {
                    @Override
                    public void run() {
                        while(!correndoAinda()) {
    ...
    private boolean correndoAinda() {
        for(Competidor competidor : competidores) {
            if (competidor.estaCorrendo()) {
                if(competidor.getDistanciaPercorrida() == distanciaDaCorrida){
                    competidor.prepararParaNovaCorrida(distanciaDaCorrida);
                }
                return false;
            }
